...
if($_SERVER['REQUEST_METHOD'] == 'GET'){
    ...
    if(isset($_GET['lastId'])){
        ...
        // On écrit la requête
        $sql = 'SELECT `messages`.`id`, `messages`.`message`, `messages`.`users_id`, `messages`.`created_at`, `users`.`pseudo` FROM `messages` LEFT JOIN `users` ON `messages`.`users_id` = `users`.`id`'.$filtre.' ORDER BY `messages`.`id`;';
